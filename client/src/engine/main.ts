...
export async function bootEngine(container: HTMLElement): Promise<Engine> {
  ...
  if (!browserGate()) throw new Error("Browser Gate Failed");
  ...
  const engine = await Engine.create(params, hooks, container);
  ...
  await buildScene('world', ctx); // Always load the world
  ...
  (window as any).laasEngine = engine;
  engine.start();
